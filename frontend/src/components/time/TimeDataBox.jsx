...
export default function TimeBox({hour, setHour}) {
  const [data, setData] = useState();

  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/time/${hour}`)
    .then((res) => res.json())
    .then((data) => {
    ...
    <div className="w-full h-full flex flex-col lg:justify-between">
      ...
      <div className="flex w-full h-full bg-[#FABCCD] rounded-[20px] p-8 flex flex-col gap-y-4">
        ...
        <div className="flex justify-between gap-x-4">
          <h1 className="text-md font-extrabold md:text-2xl">Time Dashboard</h1>
          <div className="flex gap-2 items-center">
            <p className="text-md">select time: </p>
            <SelectHour setHour={setHour}/>
          </div>
        </div>
